...
if not os.path.exists(modelspath):
    ...
    raise RuntimeError("'modelspath' variable not correctly set!")
...
def torch_warp(tensorInput, tensorFlow):
    ...
    if str(tensorFlow.size()) not in Backward_tensorGrid[device_id]:
            tensorHorizontal = torch.linspace(-1.0, 1.0, tensorFlow.size(3), dtype=tensorInput.dtype).view(1, 1, 1, tensorFlow.size(3)).expand(tensorFlow.size(0), -1, tensorFlow.size(2), -1)
            tensorVertical = torch.linspace(-1.0, 1.0, tensorFlow.size(2), dtype=tensorInput.dtype).view(1, 1, tensorFlow.size(2), 1).expand(tensorFlow.size(0), -1, -1, tensorFlow.size(3))
            if device == "cuda":
                Backward_tensorGrid[device_id][str(tensorFlow.size())] = torch.cat([ tensorHorizontal, tensorVertical ], 1).cuda().to(device_id)
            else:
                Backward_tensorGrid[device_id][str(tensorFlow.size())] = torch.cat([ tensorHorizontal, tensorVertical ], 1).to("cpu")
...
class ME_Spynet(nn.Module):
    ...
    def forward(self, im1, im2):
        ...
        flowfileds = torch.zeros(zeroshape, dtype=im1.dtype, device=device_id)
