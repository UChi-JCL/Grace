...
class SpatiallyAdaptiveCompression(ScaleHyperprior):
    def __init__(self, N=192, M=192, sft_ks=3, prior_nc=64, frozen=False, **kwargs):
        ...
        base_down = 32
        crop_sz = 256
        num_features_in = M * (crop_sz // base_down) * (crop_sz // base_down)
        num_features_out = num_features_in // 2
        
        print("Allocating for line1")
        self.line1 = nn.Sequential(
                nn.Linear(num_features_in, num_features_out),
                nn.LeakyReLU(inplace=True)
            )

        print("Allocating for line2")
        self.line2 = nn.Sequential(
                nn.Linear(num_features_out, num_features_in),
                nn.LeakyReLU(inplace=True)
            )
        change_list.extend(["line1", "line2"])
    ...
    def load_from_state_dict(self, state_dict):
        print("... loading from another state dict!")
