...
class SpatiallyAdaptiveCompression(ScaleHyperprior):
    def __init__(self, N=192, M=192, sft_ks=3, prior_nc=64, frozen=False, **kwargs):
        super().__init__(N, M, **kwargs)
        print(" ======= INITIALIZE NETWORK WITH G_USE_SMALLER = {}".format(G_USE_SMALLER))
        ### condition networks ###
        # g_a,c
        #change_list = ["g_a12", "g_s2", "qmap_feature_g5", "qmap_feature_gs2"]
        change_list = []
        ...
        self.qmap_feature_g4 = nn.Sequential(
            conv(prior_nc, prior_nc, 5, stride=4),
            nn.LeakyReLU(0.1, True),
            conv(prior_nc, prior_nc, 1, 1)
        )
        ...
        change_list.extend(["qmap_feature_g5", "qmap_feature_g4", "qmap_feature_g3"])
        ...
        self.qmap_feature_h2 = nn.Sequential(
            conv(prior_nc, prior_nc, 3, stride=1),
            nn.LeakyReLU(0.1, True),
            conv(prior_nc, prior_nc, 1, 1)
        )
        ...
        change_list.extend(["qmap_feature_h2"])
        ...
        change_list.extend(["qmap_feature_gs0"])
        ...
        self.qmap_feature_gs2 = nn.Sequential(
            UpConv2d(prior_nc, prior_nc, 5, stride=4),
            nn.LeakyReLU(0.1, True),
            conv(prior_nc, prior_nc, 1, 1)
        )
        ...
        change_list.extend(["qmap_feature_gs2", "qmap_feature_gs3", "qmap_feature_gs4"])
        ...
        self.g_a9 = Conv2d(N, N, kernel_size=7, stride=4)
        self.g_a10 = GDN1(N)
        self.g_a11 = SFT(N, prior_nc, ks=sft_ks)

        self.g_a12 = Conv2d(N, M, kernel_size=7, stride=4)
        self.g_a13 = SFTResblk(M, prior_nc, ks=sft_ks)
        self.g_a14 = SFTResblk(M, prior_nc, ks=sft_ks)

        change_list.extend(["g_a6", "g_a7", "g_a8", "g_a9", "g_a10", "g_a11", "g_a12", "g_a13", "g_a14"])
        ...
        self.h_a3 = Conv2d(N, N, stride=1)
        self.h_a4 = SFT(N, prior_nc, ks=sft_ks)
        self.h_a5 = nn.LeakyReLU(inplace=True)

        self.h_a6 = Conv2d(N, M)
        self.h_a7 = SFTResblk(M, prior_nc, ks=sft_ks)
        self.h_a8 = SFTResblk(M, prior_nc, ks=sft_ks)

        change_list.extend(["h_a3", "h_a4", "h_a5", "h_a6", "h_a7", "h_a8"])
        ...
        self.g_s2 = UpConv2d(M, N, kernel_size=7, stride=4)
        self.g_s3 = GDN1(N, inverse=True)
        self.g_s4 = SFT(N, prior_nc, ks=sft_ks)

        self.g_s5 = UpConv2d(N, N, kernel_size=7, stride=4)
        ...
        change_list.extend(["g_s2", "g_s3", "g_s4", "g_s5", "g_s6", "g_s7", "g_s8", "g_s9", "g_s10"])
    ...
    def load_from_state_dict(self, state_dict):
        ...
        for name, param in state_dict.items():
            if name not in own_state:
                print("Skip:", name, "due to name not found")
                continue
            if param.size() != own_state[name].size():
                print("Skip:", name, "due to size mismatch:", param.size(), own_state[name].size())
                continue
...
#class SpatiallyAdaptiveCompressionWrapper(SpatiallyAdaptiveCompression):
