...
class SpatiallyAdaptiveCompression(ScaleHyperprior):
    def __init__(self, N=192, M=192, sft_ks=3, prior_nc=64, frozen=False, **kwargs):
        ...
        change_list.extend(["qmap_feature_g5", "qmap_feature_g4"])
        ...
        change_list.extend(["qmap_feature_gs2", "qmap_feature_gs3"])
        ...
        self.g_a12 = Conv2d(N, M, kernel_size=5, stride=4)
        self.g_a13 = SFTResblk(M, prior_nc, ks=sft_ks)
        self.g_a14 = SFTResblk(M, prior_nc, ks=sft_ks)

        change_list.extend(["g_a9", "g_a10", "g_a11", "g_a12", "g_a13", "g_a14"])
        ...
        self.g_s2 = UpConv2d(M, N, kernel_size=5, stride=4)
        ...
        change_list.extend(["g_s2", "g_s3", "g_s4", "g_s5", "g_s6", "g_s7"])
    ...
    def encode(self, x, qmap):
        ...
        #yq = self.entropy_bottleneck.quantize(y, "symbols", means_hat)
        yq = self.entropy_bottleneck.quantize(y, "symbols", None) #// 2
        zq = self.entropy_bottleneck.quantize(z, "symbols", z_med) #// 2
    ...
    def decode(self, strings):
        ...
        #gaussian_params = self.h_s(z_hatq)
        #scales_hat, means_hat = gaussian_params.chunk(2, 1)
        #y_hatq = self.entropy_bottleneck.dequantize(y_hat, means_hat)
    ...
    def apply_loss(self, y, loss_prob):
